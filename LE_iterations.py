                ...
                for l in range(l_max+1):
                    ...
                    for L in range(np.abs(lam-l), lam+l+1):
                        ...
                        for M in range(2*L+1):
                            for mu, m, cg_coefficient in cg[(lam, l, L)][M]:
                                data[L][:, M, nu_plus_one_count[L]:nu_plus_one_count[L]+selected_features[(lam, l, L)].shape[0]] += cg_coefficient * block_nu.values[:, mu, selected_features[(lam, l, L)][:, 0]] * block_1.values[:, m, selected_features[(lam, l, L)][:, 1]]
                                if do_gradients: gradient_data[L][:, :, M, nu_plus_one_count[L]:nu_plus_one_count[L]+selected_features[(lam, l, L)].shape[0]] += cg_coefficient * (gradients_nu.data[:, :, mu, selected_features[(lam, l, L)][:, 0]] * block_1.values[samples_for_gradients_nu][:, m, selected_features[(lam, l, L)][:, 1]].unsqueeze(dim=1) + block_nu.values[samples_for_gradients_1][:, mu, selected_features[(lam, l, L)][:, 0]].unsqueeze(dim=1) * gradients_1.data[:, :, m, selected_features[(lam, l, L)][:, 1]])  # exploiting broadcasting rules
