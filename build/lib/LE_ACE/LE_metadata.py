...
def get_le_metadata(all_species, E_max, n_max_l, E_ln, is_trace, n_trace, device):
    nu_max = len(E_max) - 1
    l_max = len(n_max_l) - 1
    n_elems = len(all_species)

    # generate b, the composite (l, a, n) index
    if is_trace:
    ...
    for l in range(l_max+1):
        ...
        if is_trace: alpha_1[(l,)] = torch.tensor(alpha_1_l, dtype=torch.long, device=device)

    b_nu_minus_one = b_1
    if is_trace: alpha_nu_minus_one = alpha_1
    for nu in range(2, nu_max+1):
        ...
        for l_tuple_nu_minus_one, LE_energies_nu_minus_one_l in LE_energies_nu_minus_one.items():
            ...
            if is_trace: alpha_nu_minus_one_l_tuple = alpha_nu_minus_one[l_tuple_nu_minus_one]

            for (l,), LE_energies_1_l in LE_energies_1.items():
                if l_tuple_nu_minus_one[-1] > l: continue
                b_1_l = b_1[(l,)]
                if is_trace: alpha_1_l = alpha_1[(l,)]
                l_tuple_nu = l_tuple_nu_minus_one + (l,)
                LE_energies[nu][l_tuple_nu] = []
                b_nu[l_tuple_nu] = []
                if is_trace: alpha_nu[l_tuple_nu] = []
                ...
                for p_nu_minus_one, LE_energy_nu_minus_one in enumerate(LE_energies_nu_minus_one_l):
                    for p_1, LE_energy_1 in enumerate(LE_energies_1_l):
                        ...
                        if is_trace and (alpha_nu_minus_one_l_tuple[p_nu_minus_one] != alpha_1_l[p_1]): continue
                        ...
                        if is_trace: alpha_nu[l_tuple_nu].append(alpha_1_l[p_1])

                if len(b_nu[l_tuple_nu]) == 0: 
                    LE_energies[nu].pop(l_tuple_nu)
                    b_nu.pop(l_tuple_nu)
                    if is_trace: alpha_nu.pop(l_tuple_nu)
                    continue
                b_nu[l_tuple_nu] = torch.stack(b_nu[l_tuple_nu])
                if is_trace: alpha_nu[l_tuple_nu] = torch.stack(alpha_nu[l_tuple_nu])
        ...
        b_nu_minus_one = b_nu
        if is_trace: alpha_nu_minus_one = alpha_nu
