...
from permutations import apply_multiplicities
...
DATASET_PATH = "../rMD17/rmd17/toluene1.extxyz" # 'datasets/methane.extxyz'
TARGET_KEY = "energy"
CONVERSION_FACTOR = 1.0  # HARTREE_TO_KCALMOL
...
print("2.0, smaller basis", flush=True)
...
E_max = [1e30, 1.5*n_train_effective, 6.0*n_train_effective**(1.0/2.0), 12.0*n_train_effective**(1.0/3.0), 22.0*n_train_effective**(1.0/4.0)]
print(E_max)
...
c = torch.linalg.solve(symm, vec)
test_predictions = X_test[:n_test] @ c
print("n_train:", n_train, "n_features:", n_feat)
print(f"Test set RMSE: {get_rmse(test_predictions[:n_test], test_energies[:n_test]).item()} [MAE: {get_mae(test_predictions[:n_test], test_energies[:n_test]).item()}]")
