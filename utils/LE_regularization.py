    ...
    for i in range(len(properties["n1"])):
        if nu == 1:
            ...
            for iota in range(1, nu+1):
                eigenvalue += E_nl[properties["n"+str(iota)][i], properties["l"+str(iota)][i]]#*np.exp(beta*nu)
        # if nu == 1: eigenvalue = eigenvalue*(r_cut**2)/(r_cut_rs**2)   # This only makes sense (maybe) for pure LE
