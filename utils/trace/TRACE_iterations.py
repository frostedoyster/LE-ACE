...
class TRACEIterator(torch.nn.Module):
    ...
    def forward(self, TRACE_nu, TRACE_1, E_max_nu_plus_one):
        ...
        nu = (len(TRACE_nu.block(0).properties.names)-1)//3  # Infer nu from the length of the history indices.
        ...
        for a_i in self.all_species:
            ...
            if nu not in self.nu_plus_one_count:
                ...
                for lam in range(lam_max+1):
                    block_nu = TRACE_nu.block(lam=lam, a_i=a_i)
                    ...
                    for l in range(l_max+1):
                        ...
                        for q_nu in range(block_nu.values.shape[-1]):
                            for q_1 in range(block_1.values.shape[-1]):
                                ...
                                if self.combined_anl[(n_nu[q_nu], l_nu[q_nu])] > self.combined_anl[(n_nu_plus_1[q_1], l)]: continue
                                ...
                                properties_list = [[block_nu.properties[name][q_nu] for name in block_nu.properties.names] + [block_1.properties[name][q_1] for name in block_1.properties.names[1:-1]] + [lam]]
