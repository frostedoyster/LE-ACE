...
def cut_to_LE(map: TensorMap, E_nl, E_max) -> TensorMap:
    all_species = np.unique(map.keys["species_center"])
    ...
    for idx, block in map.items():
        ...
        for n in block.properties["n"]:
            if E_nl[n, l] <= E_max: 
                ...
                if do_gradients: LE_gradients[:, :, :, counter_LE] = block.gradient("positions").data[:, :, :, counter_total]
...
def process_radial_spectrum(map: TensorMap, E_nl, E_max) -> TensorMap:
    ...
    LE_blocks = []
    for species_center in all_species:
...
def get_LE_expansion(structures, spline_file: string, E_nl, E_max, rcut, rs=False, do_gradients=False) -> TensorMap:
    ...
    # if ...
    calculator = rascaline.torch.Calculator(calculator)
    structures = [rascaline.torch.as_torch_system(s) for s in structures]
    for s in structures:
        s.positions.requires_grad = True
    ...
    if rs:
        spherical_expansion_coefficients = process_radial_spectrum(spherical_expansion_coefficients, E_nl, E_max)
    else:
        spherical_expansion_coefficients = cut_to_LE(spherical_expansion_coefficients, E_nl, E_max)
        
    return structures, spherical_expansion_coefficients
